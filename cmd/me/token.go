				...
				<-dbExecuteMany(
					...
					ForEach(func(i interface{}) {
						*pool = append(*pool, fetchMany(url, fmt.Sprint("tokens/", mintAddress, "/listings"), 0).
							ForEach(subscribeTokenListing(pool, mintAddress, db), logError, doNothing))
						*pool = append(*pool, fetchMany(
							url,
							fmt.Sprint("tokens/", mintAddress, "/offer_received"),
							500,
						).
							ForEach(subscribeTokenOfferReceived(pool, mintAddress, db), logError, doNothing))
						*pool = append(*pool, fetchMany(
							url,
							fmt.Sprint("tokens/", mintAddress, "/activities"),
							500,
						).
							ForEach(subscribeTokenActivites(pool, mintAddress, db), logError, doNothing))
					}, logError, doNothing)
			}, logError, doNothing))
	}
}
